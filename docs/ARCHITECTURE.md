...
- File Operations (`src/file_handler/file_operations.py`) – Moves files/folders, detects conflicts, executes overwrite behavior, runs operations off the UI thread, and reports results. On Windows, uses Shell IFileOperation for moves (with shutil fallback) to ensure Explorer refreshes immediately. Session backups live under `%APPDATA%/DesktopSorter/backups/<session-id>`; empty session directories are auto-pruned on startup and shutdown so only real backup archives persist.
...
### Supporting Services
- Logging – Standard Python logging configured via `services/logging_utils`; ensures `%APPDATA%/DesktopSorter/logs/app.log` exists with a 1 MB / 5-backup rotating handler, stamps each record with the per-run session id, and honors the `DS_LOG_LEVEL` override for both console and file output.
